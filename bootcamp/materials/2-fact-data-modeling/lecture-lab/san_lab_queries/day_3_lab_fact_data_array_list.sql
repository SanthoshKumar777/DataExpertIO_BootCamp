--------------------------------------------------
-- create table array_mteics to store monthly aggregated metrics
--------------------------------------------------
...
--------------------------------------------------
-- inserting data into array_mterics by agg metric stored in array where pos represents day from month_start
--------------------------------------------------
	...
	SELECT
		...
		CASE 
			...
			WHEN ya.metric_array IS NULL
				THEN ARRAY_FILL(0, ARRAY[EXTRACT(DAY FROM da.date - DATE_TRUNC('month', da.date))::INTEGER]) || ARRAY[COALESCE(da.num_site_hits, 0)]
		END AS metric_array
