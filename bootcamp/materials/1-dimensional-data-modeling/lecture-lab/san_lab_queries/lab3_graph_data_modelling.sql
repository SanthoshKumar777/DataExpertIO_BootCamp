--------------------------------------------------
-- create types like vertex_type
--------------------------------------------------
CREATE TYPE vertex_type 
	AS ENUM('player', 'team', 'game')
	
--------------------------------------------------
-- create table called vertices
--------------------------------------------------
...
CREATE TYPE edge_type 
	AS ENUM ('play_against', 'shares_team', 'plays_in', 'plays_ong')
...
SELECT 
	...
	json_build_object(
		...
		'pts_away', pts_away,
		'winning_team', CASE 
			WHEN home_team_wins = 1 THEN home_team_id ELSE visitor_team_id
		END
	) AS properties
FROM games;

--------------------------------------------------
-- create player vertices
--------------------------------------------------
INSERT INTO vertices
WITH player_agg AS (
	SELECT 
...
aggregated AS (
	SELECT 
		...
		ROW_NUMBER() OVER(PARTITION BY f1.player_id, f2.player_id) AS row_num
	FROM filtered f1
	JOIN filtered f2
		ON f1.game_id = f2.game_id
		AND f1.player_name <> f2.player_name
	WHERE f1.player_id > f2.player_id
	GROUP BY f1.player_id, f2.player_id,f1.team_abbreviation, f2.team_abbreviation)
SELECT 
...
SELECT 
	...
	e.properties->>'num_games' AS num_vs_games
FROM vertices v JOIN edges e
	ON v.identifier = e.subject_identifier
	AND v.type = e.subject_type
WHERE e.object_type = 'player'::vertex_type
AND CAST(v.properties->>'total_points' AS REAL) > 0 
